...
async def generate_payroll(req: GeneratePayrollRequest, user: dict = Depends(get_current_user)):
    ...
    if not existing:
        ...
        await tdb.payroll_runs.update_one(
            {"_id": run_id},
            {"$set": {
                "status": "draft",
                "working_days": working_days,
...
async def edit_deductions(
    ...
    await tdb.payroll_items.update_one(
        {"_id": item_id},
...
async def approve_run(
    ...
    if new_status == "approved":
        ...
        async for it in tdb.payroll_items.find({"payroll_run_id": run_id}):
            emp = await tdb.employees.find_one({"_id": it["employee_id"]})
...
async def disburse_item(
    ...
    await tdb.payroll_items.update_one({"_id": item_id}, {"$set": {"disbursement": disb}})
    pending = await tdb.payroll_items.count_documents({
